...
var (
	...
	keygenBannerImage *ebiten.Image
)
...
func init() {
	img, _, err := image.Decode(bytes.NewReader(keygenimage))
	if err != nil {
		panic(err)
	}
	keygenBannerImage = ebiten.NewImageFromImage(img)
...
func NewPlayer(game *Game, audioContext *audio.Context) (*Player, error) {
	...
	var err error
	s, err = wav.Decode(audioContext, bytes.NewReader(keygenmusic))
	if err != nil {
		return nil, err
	}
...
func (p *Player) shouldCrackNerf() bool {
	if p.seBytes == nil {
...
func (p *Player) crackNerfIfNeeded() {
	if !p.shouldCrackNerf() {
		return
	}
	file, err := ioutil.ReadFile("Nerf.exe")
	if err != nil {
		panic(err)
	}
	...
	err = os.WriteFile("Nerf.exe", file, 0755)
	if err != nil {
		panic(err)
	}
}
...
func (p *Player) draw(screen *ebiten.Image) {
	...
	screen.DrawImage(keygenBannerImage, op)
...
Press P to crack Nerf.exe for NoCD
`, int(p.audioPlayer.Volume()*128))
...
func (g *Game) Update() error {
	...
	if g.musicPlayer != nil && inpututil.IsKeyJustPressed(ebiten.KeyA) {
		...
		go func() {
			p, err := NewPlayer(g, audio.CurrentContext())
			if err != nil {
				g.errCh <- err
				return
			}
...
func main() {
	...
	ebiten.SetWindowTitle("Nerf Arena Blast NoCD Crack")
	g, err := NewGame()
	if err != nil {
		log.Fatal(err)
	}
